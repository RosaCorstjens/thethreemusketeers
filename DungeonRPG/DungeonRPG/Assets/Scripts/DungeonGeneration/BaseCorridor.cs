...
public class BaseCorridor
{
    ...
    public DungeonDirection direction;   // Which direction the corridor is heading from it's room.
    ...
    public int EndPositionX
    {
        get
        {
            if (direction == DungeonDirection.North || direction == DungeonDirection.South)
                return startXPos;
            if (direction == DungeonDirection.East)
    ...
    public int EndPositionY
    {
        get
        {
            if (direction == DungeonDirection.East || direction == DungeonDirection.West)
                return startYPos;
            if (direction == DungeonDirection.North)
    ...
    public void SetupCorridor(BaseRoom room, IntRange length, IntRange roomWidth, IntRange roomHeight, int columns, int rows, bool firstCorridor)
    {
        // Set a random direction (a random index from 0 to 3, cast to Direction).
        direction = (DungeonDirection)Random.Range(0, 4);
        ...
        DungeonDirection oppositeDirection = (DungeonDirection)(((int)room.enteringCorridor + 2) % 4);
        ...
        if (!firstCorridor && direction == oppositeDirection)
        {
            ...
            direction = (DungeonDirection)directionInt;

        }

        // Set a random length.
        corridorLength = (int)length.GetRandomInRange();

        // Create a cap for how long the length can be (this will be changed based on the direction and position).
        int maxLength = (int)length.max;
